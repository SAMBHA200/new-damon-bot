...
client.on("message", async message => {
  if (message.channel.type === "dm") {
    ...
    if (active === null) {
      ...
      channel = await guild.channels.create(
        ...
        { type: "text", reason: `Modmail created ticket #${actualticket}.` }
      );

      channel.setParent(config.ticketCategory);

      channel.setTopic(
        `#${actualticket} (Open) | ${config.prefix}complete to close this ticket | Modmail for ${message.author.username}`
      );
      ...
      message.author.send(
        `Hello ${author.username}, your ticket #${actualticket} has been created.`
      );
    ...
    var whatWeWant = msg
      ...
      .replace("@here", `[here]`); // idk if that's useful since we're blocking mentions
    ...
    if (isPaused === true) {
      return message.channel.send(
        "Sorry, but your ticket is currently paused. I'll message you back when the support team unpause it."
      );
    }
  ...
  if (support) {
    ...
    if (message.content.startsWith(`${config.prefix}reply`)) {
      ...
      if (isPause === true)
        return message.channel.send(
          "This ticket already paused. Unpause it to continue."
        );

      if (isBlock === true)
        return message.channel.send(
          "The user is blocked. Unblock them to continue or close the ticket."
        );
    ...
    if (message.content === `${config.prefix}pause`) {
      ...
      var suspend = new Discord.MessageEmbed()
        ...
        .setDescription(
          `⏸️ This thread has been **locked** and **suspended**. Do \`${config.prefix}continue\` to cancel.`
        )
      ...
      return client.users.cache
        .get(support.targetID)
        .send(
          "Your ticket has been paused. We'll send you a message when we're ready to continue."
        );
    }
    ...
    if (message.content === `${config.prefix}continue`) {
      ...
      if (isPause === null || isPause === false)
        return message.channel.send("This ticket was not paused.");
      ...
      return client.users.cache
        .get(support.targetID)
        .send("Hi! Your ticket isn't paused anymore. We're ready to continue!");
    }
    ...
    if (message.content.startsWith(`${config.prefix}block`)) {
      ...
      if (isBlock === true)
        return message.channel.send("The user is already blocked.");
      ...
      var c = new Discord.MessageEmbed()
        ...
        .setDescription(
          "⏸️ The user has been blocked from the modmail. You may now close the ticket or unblock them to continue."
        )
    ...
    async function end(userID) {
      ...
      return client.users.cache
        .get(support.targetID)
        .send(
          `Your ticket #${actualticket} has been closed! If you wish to open a new ticket, feel free to message me.`
        );
    }
  }
});
...
client.on("message", async message => {
  if (message.content.startsWith(`${config.prefix}unblock`)) {
    if (
      message.guild.member(message.author).roles.cache.has(config.roles.mod)
    ) {
      var args = message.content.split(" ").slice(1);
      ...
      client.users
        ...
        .then(async user => {
          ...
          if (data === true) {
            ...
            return message.channel.send(
              `Successfully unblocked ${user.username} (${user.id}) from the modmail service.`
            );
          } else {
            return message.channel.send(
              `${user.username} (${user.id}) is not blocked from the modmail at the moment.`
            );
          }
        })
        .catch(err => {
