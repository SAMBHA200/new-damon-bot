...
client.getSettings = async guild => {
  ...
  if (guild)
    await client
      ...
      .then(result => (guildSettings = result[0]));

  await client
    .getGuildSettings(0)
    .then(result => (defaultSettings = result[0]));
...
client.updateGuildSetting = (guild, key, value) => {
  db.run(
    `UPDATE guilds SET ${key} = ? WHERE id = ?`,
    [value, guild.id],
    async function(err, rows) {
      if (err)
        return console.log(
          "An error occurred while trying to update guild settings.",
          err
        );
    }
  );
...
client.addBan = user => {
  db.run(
    "INSERT OR IGNORE INTO banlist (id) VALUES (?)",
    [user],
    async function(err, rows) {
      if (err)
        return console.log(
          "An error occurred while trying to add into banlist",
          err
        );
    }
  );
...
client.on("guildCreate", guild => {
  console.log(
    `New guild added ${client.user.tag}: ${guild.name} (${
      guild.id
    }) with ${guild.memberCount - 1} members`
  );

  db.run("INSERT INTO guilds (id) VALUES (?)", [guild.id], function(
    err,
    rows
  ) {});
});
...
client.on("guildMemberAdd", async member => {
  ...
  if (banlist.includes(member.id))
    return member.ban({
      days: 1,
      reason: "Auto-Banned by Protection - Bot Detected"
    }); // if account ID is in the banlist, it'll get bananed from the server

  const settings = await client.getSettings(member.guild).then(r => r);

  if (settings.type === "password") {
    const msg = await member.send(
      `**${member.guild.name}** is password protected. Please enter the password.`
    );
    ...
    if (response.includes(settings.password)) {
      ...
      msg.reply(
        `Wrong password. Type \`${settings.prefix}password\` for another try.`
      );
    }
  } else if (settings.type === "captcha") {
    member.send(
      `This server is protected by captcha. Type \`${
        settings.prefix
      }verify\` in <#${
        member.guild.channels.find(c => c.name === settings.verificationChannel)
          .id
      }>`
    );
  }
...
client.on("message", async message => {
  ...
  if (message.guild) {
    const verChan = message.guild.channels.find(
      c => c.name === settings.verificationChannel
    );

    if (verChan) {
      if (
        message.channel.name === verChan.name &&
        settings.autoban === "true"
      ) {
        if (message.content.includes("http")) {
          // bots usually post links, this will check if message contains a link, and ban the account but also add it to the global banlist
  ...
  if (message.content.startsWith(`${settings.prefix}verify`)) {
    ...
    if (message.channel.type === "dm")
      return message.channel.send("This command does not work via DM.");

    const captcha = await svgCaptcha.create(options);

    const attachment = new Discord.Attachment(
      await client.img(captcha.data).then(r => r),
      "captcha.png"
    );
    ...
    const m1 = await message.author
      .send("Generating captcha...")
      .catch(async () => {
        const delmsg = await message.reply(
          "I'm unable to send you a direct message. Make sure to allow direct messages from server members in the privacy settings!"
        );
    ...
    if (!response)
      return message.author.send("Expired.").catch(async () => {
        const delmsg = await message.reply(
          "I'm unable to send you a direct message. Make sure to allow direct messages from server members in the privacy settings!"
        );
    ...
    if (response.includes(captcha.text)) {
      ...
      const role = await message.guild.roles.find(
        r => r.name.toLowerCase() === settings.role.toLowerCase()
      );

      if (role) message.member.addRole(role.id);
    } else {
      const logs = message.guild.channels.find(
        c => c.name === settings.logsChannel
      );

      if (logs) logs.send(`Failed captcha attempt by ${message.author.tag}`);

      return msg.reply(
        `Wrong captcha! Try again by typing \`${
          settings.prefix
        }verify\` again in <#${
  ...
  if (message.content.startsWith(`${settings.prefix}password`)) {
    ...
    const msg = await message.member.send(
      `**${message.guild.name}** is password protected. Please enter the password.`
    );
    ...
    if (response.includes(settings.password)) {
      ...
      const role = await message.guild.roles.find(
        r => r.name.toLowerCase() === settings.role
      );
  ...
  if (message.content.startsWith(`${settings.prefix}config`)) {
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(
        "Sorry, but you must have the `ADMINISTRATOR` permission to use this command."
      );
    ...
    if (!args.length) {
      const embed = new RichEmbed()
        ...
        .addField(
          "Protection Type",
          `You can choose between captcha or server password\n**Current**: \`${
            settings["type"]
        ...
        .addField(
          "Password",
          `Server password. Only valid if protection type is set to password.\n**Current**: \`${settings.password}\`\n\`${settings.prefix}config password [new-password]\``
        )

        .addField(
          "Verification Channel",
          `This is the channel where new members will be able to type \`${
            settings.prefix
          }verify\`\n**Current**: ${settings["verificationChannel"]}\n\`${
            settings.prefix
          }config verificationChannel [channel-name]\``
        )

        .addField(
          "Verified Role",
          `This is the role that new members will receive after they succcessfully pass the captcha\n**Current**: ${
            settings["role"]
        ...
        .addField(
          "Logs Channel",
          `This is the channel that will log all verifications\n**Current**: ${
        ...
        .addField(
          "AutoBan",
          `This option will autoban detected bots\n**Current**: ${settings.autoban}\n\`${settings.prefix}config autoban [true | false]\``
        )

        .addField(
          "Prefix",
          `Prefix of the bot in this server\n**Current**: ${
            settings["prefix"]
    ...
    if (!key)
      return message.reply("Please specify a setting that you want to change.");

    if (!settings[key]) return message.reply("That is not a setting.");

    if (key.toLowerCase() === "id")
      return message.reply(
        "You can't edit this setting, it's your server's ID."
      );

    if (!args[1].length) return message.reply("Please specify a new value");

    if (args[1] === settings[key])
    ...
    if (key.toLowerCase() === "type" && !types.includes(args[1]))
      return message.reply(
        "Protection type must be either `password` or `captcha`"
      );

    if (key.toLowerCase() === "autoban" && !autoban.includes(args[1]))
      return message.reply(
        "Autoban must be either `true` for enabled or `false` for disabled"
      );
    ...
    if (
      ...
      return message.reply("Couldn't find that role in this server!");

    if (
      key.toLowerCase() === "verificationchannel" &&
      !message.guild.channels.find(
        r => r.name.toLowerCase() === args[1].toLowerCase()
      )
    )
  ...
  if (message.content.startsWith(`${settings.prefix}setup`)) {
    ...
    const e1 = new RichEmbed()
      ...
      .setDescription(
        `Welcome to <@${client.user.id}>'s setup. To get started, type \`captcha\` for captcha protection or \`password\` for password protection.`
      );
    ...
    if (!re1)
      return message.channel.send(
        "Setup waiting period expired. Type the setup command to go through it again!"
      );
    ...
    if (re1 === "captcha") {
      const e2 = new RichEmbed()
        ...
        .setDescription(
          `Please enter the name of the verification channel. This is the channel where new members will have to type \`${settings.prefix}verify\` to get access to the server.`
        );
      ...
      const chan = message.guild.channels.find(
        c => c.name.toLowerCase() === re2.toLowerCase()
      );

      if (!chan) {
        message.reply(
          "Can't find that channel! Make sure you only type the channel name!"
        );
      } else {
      ...
      const e2 = new RichEmbed()
        ...
        .setDescription(
          `Please enter the name of the verification channel. This is the channel where new members will have to type \`${settings.prefix}password\` incase they type the wrong password via DM.`
        );
      ...
      const e3 = new RichEmbed()
        ...
        .setDescription(
          "Please enter the server password you want. New members will be asked for this password via DM to get access to the server."
        );
    ...
    const e4 = new RichEmbed()
      ...
      .setDescription(
        "Please enter the name of the verified role. This protection is based on permissions, this role will have normal permissions that will give access to most of the server to new members, while the @ everyone must be denied of all permissions except for the verification channel."
      );

    const re4 = await client.awaitReply(message, e4, 60000);

    const role = message.guild.roles.find(
      r => r.name.toLowerCase() === re4.toLowerCase()
    );
    ...
    if (!role) {
      message.channel.send(
        "Couldn't find that role. Make sure you enter only the name of the role."
      );
    } else {
      ...
      if (role.position > message.guild.me.highestRole.position)
        message.channel.send(
          "Move my role higher up in the role hierarchy or I won't be able to give out this role!"
        );
    }
    ...
    const e5 = new RichEmbed()
      ...
      .setDescription(
        `Last but not least, would you like <@${client.user.id}> to deny permissions to all channels for @ everyone except for the Verification Channel? Reply with \`Yes\` or \`No\``
      );
    ...
    const e6 = new RichEmbed()
      ...
      .setDescription(
        `Basic setup complete! You may review your configuration via the \`${settings.prefix}config\` command where you can also modify other settings that were excluded from the setup!`
      );
