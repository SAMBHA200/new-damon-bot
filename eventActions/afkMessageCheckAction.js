...
class afkMessageCheckAction {
  static async checkIfUserIsAFK(message) {
      ...
      if (
        ...
        if (reaction.emoji.name === "✅") {
          ...
          reaction.message
            .delete()
            .catch(() =>
              console.log("Tried deleting afk message that was already deleted")
            );
    ...
    const noLongerAFKMessage = new Discord.MessageEmbed()
      ...
      .addField(
        "Are you back?",
        "Go to <#415261994887938048> and run `!afk` again to turn off AFK!",
        true
      )

      .addField(
        "If you are not back!",
        "Ignore this message. It will be deleted after 15 seconds.",
        true
      )
