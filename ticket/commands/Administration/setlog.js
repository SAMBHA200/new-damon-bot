...
exports.run = async (bot, message, args, functions) => {
  ...
  if (!channel || channel.type !== "text")
    return functions.errorEmbed(
      message,
      message.channel,
      "Veuillez un salon texte."
    );
  let channelFetched = message.guild.channels.cache.find(
    c => c.id === channel.id
  );
  if (!channelFetched || channelFetched.type !== "text")
    return functions.errorEmbed(
      message,
      message.channel,
      "Veuillez un salon texte."
  ...
  functions.successEmbed(
    message,
    message.channel,
    `Le salon de \`logs\` a été défini sur ${channelFetched}`
  );
};
