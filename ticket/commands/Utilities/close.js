  ...
  if (message.author.id === db.get(`ticket.${message.channel.name}.user`)) {
    ...
    if (!support)
      return functions.errorEmbed(
        message,
        message.channel,
        "Le rôle `Ticket Support` n'existe pas, veuillez le créer."
      );
    ...
    if (args[0] === "force") {
      ...
      if (bot.users.cache.get(db.get(`ticket.${message.channel.name}.user`)))
        bot.users.cache
          .get(db.get(`ticket.${message.channel.name}.user`))
          .send(embed1)
          .catch(e => {
            console.log(e);
          });
      ...
      if (!message.guild.member(message.author).roles.cache.has(support.id))
        return functions.errorEmbed(
          message,
          message.channel,
          "Désolé, vous n'avez pas le rôle `Ticket Support`."
        );
