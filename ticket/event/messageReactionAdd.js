...
module.exports = async (bot, reaction, user) => {
  ...
  let logsChannel = message.guild.channels.cache.find(
    c => c.id === db.get(`logs_${message.guild.id}`)
  );
  ...
  let success = new Discord.MessageEmbed()
    ...
    .setDescription(
      `Veuillez expliquer la raison de votre demande. Un membre de l'équipe prendra en charge votre ticket sous peu.`
    );
  ...
  if (
    ...
    message.embeds[0].title === "Système de Ticket" &&
    message.embeds[0].description === "Réagissez avec 🎟️ pour créer un ticket."
  ) {
    if (reaction.emoji.name === "🎟️") {
      if (
        ...
        if (!categoria)
          categoria = await message.guild.channels
            .create("tickets", { type: "category", position: 1 })
            .catch(e => {
              return functions.errorEmbed(
                message,
                message.channel,
        ...
        message.guild.channels
          ...
          .then(channel => {
            let createdEmbed = new Discord.MessageEmbed()
              ...
              .setDescription(
                `Un utilisateur à ouvert un ticket et attend qu'on s'occupe de sa demande.`
              )
  ...
  if (
    message.embeds.length === 1 &&
    message.embeds[0].title === "🎟️ | Ticket Terminé" &&
    message.embeds[0].description ===
      `Réagissez avec \\🗑️ pour fermer le ticket ou ne réagissez pas si vous avez d'autres demandes.`
  ) {
    if (reaction.emoji.name === "🗑️") {
